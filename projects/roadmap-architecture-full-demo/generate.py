#!/usr/bin/env python3
"""Roadmap Architecture full demo generator (data-driven + dynamic loaders)."""
...
LOADERS_DIR = ROOT / "skills" / "pptx" / "roadmap-architecture" / "loaders"
...
OUTPUT = Path(__file__).resolve().parent / "roadmap-architecture-full-demo.pptx"
...
def load_all_loaders():
    files = sorted(LOADERS_DIR.glob("ra_*.py"))
    print(f"[Info] 找到 {len(files)} 个 RA 装载器模块")
...
def add_cover_slide(ctx, cfg):
    ...
    title_ph = next((s for s in slide.shapes if getattr(s, "is_placeholder", False) and s.placeholder_format.idx == 0), None)
    if title_ph and title_ph.has_text_frame:
        tf = title_ph.text_frame
        tf.clear()
        p = tf.paragraphs[0]
        p.text = cfg.get("title", "Roadmap Architecture Demo")
        p.font.size = Pt(52)
        p.font.bold = True
    ...
    if subtitle_ph and subtitle_ph.has_text_frame:
        ...
        tf.clear()
        p = tf.paragraphs[0]
        p.text = cfg.get("subtitle", "Data-driven loader architecture")
        p.font.size = Pt(14)
...
def my_slides(ctx):
    ...
    for section in sections:
        section_title = section.get("title", "Roadmap Section")
...
if __name__ == "__main__":
    print("=" * 60)
    print("Roadmap Architecture 全量生成器")
    print("=" * 60)
