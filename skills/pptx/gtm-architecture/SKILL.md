...
2. 圆角框半径 <= 6pt，避免默认大圆角。
3. 每条连接线必须承载语义，避免纯装饰。
4. 色彩必须从当前母版主题动态读取（ctx.colors / ctx.palette），禁止在 loader 中硬编码品牌色；仅图例语义专用色可固定。
5. 所有价值声明需有可验证数据或来源。
6. 竞争和能力声明避免绝对化措辞，需可辩护。
7. 所有布局常量按 10.0" x 5.625" 逻辑画布定义，渲染时必须按当前母版尺寸动态适配，不得假设固定画幅。
8. 深色主题必须使用高对比策略，至少分离边线/连接线颜色与正文颜色（如 stroke 与 ink），避免浅底浅字或深底深线。
9. 对流程、依赖、时序、关系类图表，连接器默认使用曲线样式（CURVE）；仅当图类规范明确要求其他样式时可例外。
10. 发布候选稿必须通过业务内容越界检测，raw overflow 与 business overflow 均为 0。
...
- [ ] 价值类图表包含基线和目标口径
- [ ] 颜色来源于母版主题并满足语义映射
- [ ] 深色主题下文字、线条、分组背景具备可读对比度
- [ ] 流程/依赖类连线默认使用曲线样式（若适用）
- [ ] 业务越界检测通过（raw=0 且 business=0）
- [ ] 页面可在 30 秒内被目标受众理解