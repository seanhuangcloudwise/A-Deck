...
8. 所有布局常量按 10.0" x 5.625" 逻辑画布定义，渲染时必须按当前母版尺寸动态适配，不得假设固定画幅。
9. 深色主题必须采用高对比策略，分离线条/边框颜色与正文颜色，避免可读性下降。
10. 流程、依赖、关键路径、时间关系等连接器默认使用曲线样式（CURVE）；仅图类规范明确要求时可例外。
11. 发布候选稿必须通过业务越界门禁：raw overflow = 0 且 business overflow = 0。
...
- [ ] 深色主题下线条、文字、泳道/分组背景是否具备可读对比度？
- [ ] 流程/依赖类连线是否默认采用曲线样式（若适用）？
- [ ] 业务越界检测是否通过（raw=0 且 business=0）？
